...
| `/kimi-web [--port <n>]` | Start `kimi web` detached on loopback (default port 58627) and print the URL + bearer token. Never uses `--dangerous-bypass-auth`. |
...
- `/kimi-web` binds loopback only. The bearer token it prints is a secret:
